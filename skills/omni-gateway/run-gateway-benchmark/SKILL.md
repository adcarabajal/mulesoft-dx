...
in `REPORT.md` alongside this file. Read it before extending the harness; this skill assumes
it as background.
...
- `install-omni-gateway` — install Flex on Linux/Docker/Kubernetes (this skill provisions its
  own EKS install for benchmarking; use `install-omni-gateway` for production deployments)
- `validate-gateway-config` — validate the rendered `flex-config.yaml` before `make deploy-flex`
- `inspect-gateway-logs` — parse Flex pod logs when a benchmark surfaces unexpected failures
- `diagnose-gateway-error` — triage non-harness gateway issues observed during a run
