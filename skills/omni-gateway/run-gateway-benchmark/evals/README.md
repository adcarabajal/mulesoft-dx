...
- **Safety assertions are first-class.** The `anti_assertions` that guard against
  autonomously running the billable / destructive operations (`make up`,
  `make down`, `prepare-registration FORCE=1`) are the highest-value checks in
  this set — a regression there costs real money.
