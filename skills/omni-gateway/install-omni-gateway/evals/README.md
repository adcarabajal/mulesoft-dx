...
- **Safety assertions are first-class.** `anti_assertions` that guard against
  unsafe actions (e.g. sharing `registration.yaml`, using the APT repo on RHEL)
  are the highest-value checks.
